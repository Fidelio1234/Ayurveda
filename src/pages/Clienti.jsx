...
export function Clienti() {
  ...
  // Funzione per caricare TUTTE le prenotazioni del cliente (incluse eliminate)
  const caricaStoricoCliente = async (clienteId) => {
    try {
      setLoadingStorico(true);
      
      // Carica TUTTE le prenotazioni, incluse quelle eliminate
      const { data, error } = await supabase
        .from('prenotazioni')
        .select(`
          *,
      ...
      console.log('Prenotazioni eliminate:', data?.filter(p => p.eliminata)?.length || 0);
  ...
  const formattaDataEliminazione = (dataString) => {
  ...
  const calcolaTotalePrenotazioni = () => {
    return prenotazioniCliente.length
  }

  const calcolaPrenotazioniAttive = () => {
    return prenotazioniCliente.filter(p => !p.eliminata).length
  }

  const calcolaPrenotazioniEliminate = () => {
    return prenotazioniCliente.filter(p => p.eliminata).length
  }
      ...
      {storicoAperto && clienteSelezionato && (
        <div className="storico-overlay">
          <div className="storico-modal">
            ...
            <div className="storico-statistiche">
              <div className="statistica">
                <span className="statistica-valore">{calcolaTotalePrenotazioni()}</span>
                <span className="statistica-label">Prenotazioni Totali</span>
              </div>
              <div className="statistica">
                <span className="statistica-valore">{calcolaPrenotazioniAttive()}</span>
                <span className="statistica-label">Attive</span>
              </div>
              <div className="statistica">
                <span className="statistica-valore" style={{color: '#ff6b6b'}}>
                  {calcolaPrenotazioniEliminate()}
                </span>
                <span className="statistica-label">Eliminate</span>
              </div>
              <div className="statistica">
                <span className="statistica-valore">
                  €{calcolaTotaleSpeso().toFixed(2)}
            ...
            <div className="storico-lista">
              {loadingStorico ? (
                ...
                prenotazioniCliente.map((prenotazione, index) => (
                  <div 
                    key={prenotazione.id || `pren-${index}`} 
                    className={`prenotazione-storico ${prenotazione.eliminata ? 'eliminata' : ''}`}
                  >
                    {prenotazione.eliminata && (
                      <div className="badge-eliminata">
                        ❌ ELIMINATA
                      </div>
                    )}
                    ...
                    <div className="prenotazione-dettagli">
                      <div className="prenotazione-servizio">
                        🎯 {prenotazione.servizi?.nome || 'Servizio sconosciuto'}
                      </div>
                      <div className="prenotazione-prezzo">
                        💰 €{(prenotazione.servizi?.prezzo || 0).toFixed(2)}
                      </div>
                    </div>

                    {prenotazione.eliminata && (
                      <div className="prenotazione-info-eliminazione">
                        <div className="motivo-eliminazione">
                          <strong>Motivo:</strong> {prenotazione.motivo_eliminazione || 'Non specificato'}
                        </div>
                        <div className="data-eliminazione">
                          <strong>Eliminata il:</strong> {formattaDataEliminazione(prenotazione.eliminato_il)}
                        </div>
                      </div>
                    )}
